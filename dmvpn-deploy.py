...
def config_ntp(task):
    ntp_template = task.run(task=template_file, name="Building NTP Configuration", template="ntp.j2",
                            path="./templates")
...
def config_nhrp(task):
    nhrp_template = task.run(task=template_file, name="Building NHRP Configuration", template="nhrp.j2",
                             path="./templates")
...
def config_eigrp(task):
    eigrp_template = task.run(task=template_file, name="Building EIGRP Configuration", template="eigrp.j2",
                              path="./templates")
